...
- `import`
- `generate-masks` (deprecated; use Bone Contouring)
- `register`
- `stackcorrect`
- `transform`
- `fill`
...
`timelapse run` consumes supplied masks/ROIs. Use a `REGMASK`, `FULL_MASK`, or other ROI as the registration/support mask, provide a segmentation when the selected remodelling method requires binary bone state, and add any analysis ROI masks you want summarized. Prepare missing masks with Bone Contouring before running Timelapsed.
...
Timelapsed consumes existing registration masks, analysis ROIs, and bone segmentations.
Generate or edit these with Bone Contouring or scanner-side tools before running Timelapsed.
...
`timelapse run` is designed to resume instead of recomputing finished work. Masks/ROIs are treated as supplied inputs and are not regenerated by the remodelling workflow.

It skips stages when the expected outputs already exist:

- imported sessions
- imported masks/ROIs and segmentation
- baseline transforms
- final transforms
